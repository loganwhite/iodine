...
help() {
	...
	fprintf(stderr, "Usage: %s [-x] [-v] [-h] [-f] [-r] [-u user] [-t chrootdir] [-d device] "
			"[-P password] [-m maxfragsize] [-M maxlen] [-T type] [-O enc] [-L 0|1] [-I sec] "
			"[-z context] [-F pidfile] [nameserver] topdomain\n", __progname);
	...
	fprintf(stderr, "  -x time out interval without any data transfer\n");
...
main(int argc, char **argv)
{
	...
	while ((choice = getopt(argc, argv, "46vfhru:t:d:R:P:m:M:F:T:O:L:I:x:")) != -1) {
		switch(choice) {
		case '4':
	...
	client_set_nodatatimeout(nodatatimeout);
