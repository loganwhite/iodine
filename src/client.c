...
static int nodatatimeout;		/* Set No data timeout interval, the default value is 60sec */
static int lazymode;
...
void
client_set_nodatatimeout(int nodata_timeout) {
	nodatatimeout = nodata_timeout;
}
...
tunnel_dns(int tun_fd, int dns_fd)
{
	...
	if (read == 5 && !strncmp("BADIP", buf, 5)) {
		warnx("BADIP: Server rejected sender IP address (maybe iodined -c will help), or server kicked us due to timeout. Will exit if no downstream data is received in %d seconds.",nodatatimeout);
		return -1;	/* nothing done */
	}
...
client_tunnel(int tun_fd, int dns_fd)
{
	...
	while (running) {
		...
		i = select(MAX(tun_fd, dns_fd) + 1, &fds, NULL, NULL, &tv);
 		...
 		if (lastdownstreamtime + nodatatimeout < time(NULL)) {
 			warnx("No downstream data received in %d seconds, shutting down.",nodatatimeout);
 			running = 0;
 		}
